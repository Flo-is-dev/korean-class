...
import { matchesBook } from '../lib/books';
import { SelectField } from '../components/SelectField';
import { buildCards, categoryName, categoryOf, CATEGORIES, lessonTag, searchText, shuffle } from '../lib/cards';
import type { Book, BookFilter, Card, CardRow, Category, LibraryFilters } from '../types';

export function LibraryPage({ books, rows, filters, admin, busy, onFilters, onStart, onDelete }: {
  books: Book[] | null; rows: CardRow[]; filters: LibraryFilters; admin: boolean; busy: boolean; onFilters: (filters: LibraryFilters) => void; onStart: (cards: Card[]) => void; onDelete: (ids: string[]) => void;
}) {
  const all = useRef<HTMLInputElement>(null);
  const query = searchText(filters.query);
  const matchingOtherFilters = rows.filter(row => (filters.cat === 'all' || categoryOf(row) === filters.cat) && searchText(`${row.fr} ${row.ko} ${lessonTag(row)}`).includes(query));
  const visible = matchingOtherFilters.filter(row => books === null || matchesBook(row, filters.book));
  const bookOptions: [string, string][] = [['all', `Tous les livres (${matchingOtherFilters.length})`]];
  for (const book of books ?? []) {
    bookOptions.push([`book:${book.code}`, `${book.code} · ${book.label} (${matchingOtherFilters.filter(row => row.book === book.code).length})`]);
  }
  if (rows.some(row => row.book === null)) {
    bookOptions.push(['none', `Sans livre (${matchingOtherFilters.filter(row => row.book === null).length})`]);
  }
